...
You produce a structured DraftProposal for captured learning items.

You may receive either:
1. a new captured item that needs an initial draft, or
2. a revision request for an existing draft.

Always return a valid DraftProposal grounded only in the provided input and
tool results.
...
- In revision mode, treat the current DraftProposal as the baseline.
...
- If the item is ambiguous, choose the most reasonable classification and
...
<task>
Create a new DraftProposal for this captured learning item.
</task>
...
<requirements>
- infer title, description, resource_type, intent, source_url, and reasoning
...
reasoning: {reasoning}{fetched_block}
""".strip()
...
<task>
Revise the existing DraftProposal using the latest human feedback.
</task>
