...
- **`notion/writer.py`** — `NotionWriter` creates project pages (with page-body children: bookmark, note, key points, to-do tasks, provenance callout) and task pages in Notion databases. Also performs duplicate detection (`find_project_by_source`), schema fitting (`_fit_to_schema` drops properties missing from the target database, and renames `Project type` to legacy `Type` when needed), undo (`archive_pages`), and schema health reporting (`describe_schema` for the healthcheck). Sets page icons from project type, due dates from priority, and topic-based tags.
